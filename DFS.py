...
def dfs(initial_state):
    graph = pydot.Dot(graph_type='digraph', label="8 Puzzle State Space (DFS) \n Sansrit Paudel (35)", fontsize="30", color="red",
    ...
    while not(s.isEmpty()):
        ...
        if node.parent:
            graph.add_edge(pydot.Edge(node.parent.graph_node,
                                      node.graph_node, label=str(node.action)))
        if node.depth < 5:
                ...
                if child.state not in explored:
                    ...
                    if child.goal_test():
                        ...
                        graph.add_edge(pydot.Edge(
                            child.parent.graph_node, child.graph_node, label=str(child.action)))
                        draw_legend(graph)
                        graph.write_png('grah_.png')
